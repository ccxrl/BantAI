                ...
                while not self.stop_event.is_set():
                    if ser.in_waiting > 0:
                        ...
                        try:
                            ...
                            if all(x in data for x in ['BPM=', 'Avg BPM=', 'SpO2=']):
                                # Extract BPM
                                bpm_match = re.search(r'BPM=(\d+)', data)
                                avg_bpm_match = re.search(r'Avg BPM=(\d+)', data)
                                spo2_match = re.search(r'SpO2=(\d+)', data)

                                if bpm_match and avg_bpm_match and spo2_match:
                                    current_bpm = int(bpm_match.group(1))
                                    avg_bpm = int(avg_bpm_match.group(1))
                                    spo2 = int(spo2_match.group(1))

                                    # Update current heart rate and SpO2
                                    self.current_heart_rate = avg_bpm  # Using average BPM
                                    self.current_spo2 = spo2

                                    # Optional: Print for debugging
                                    print(f"Current BPM: {current_bpm}, Avg BPM: {avg_bpm}, SpO2: {spo2}")

                            # Emit the received data to update text area
                            self.signals.data_received.emit(data)

                        except Exception as parse_error:
                            # Handle parsing errors
                            print(f"Error parsing serial data: {parse_error}")
                            self.signals.data_received.emit(f"Parse Error: {data}")
        ...
        for emotion in emotions:
            ...
            if emotion != self.last_emotion and (self.last_advice_time is None or
                ...
                try:
                    ...
                    # Record heart rate and SpO2 if available from serial data
                    if hasattr(self, 'current_heart_rate') and hasattr(self, 'current_spo2'):
                        try:
                            heart_rate_record_id = self.db.add_heart_rate_record(
                                user_id=self.user_id,
                                heart_rate=self.current_heart_rate,
                                spo2=self.current_spo2
                            )
                            print(f"Recorded heart rate with ID: {heart_rate_record_id}")

                        except Exception as hr_e:
                            print(f"Error recording heart rate: {hr_e}")
